...
interface ExecutiveSummarySBARData {
  situation?: string;
  background?: string;
  assessment?: string;
...
interface ExecutiveSummarySBARProps {
  data: ExecutiveSummarySBARData;
  isEditMode?: boolean;
  onChange?: (newData: ExecutiveSummarySBARData) => void;
}

export const ExecutiveSummarySBAR: React.FC<ExecutiveSummarySBARProps> = ({ 
  data,
  isEditMode = false,
  onChange
}) => {
  ...
  const updateField = (field: keyof ExecutiveSummarySBARData, value: any) => {
    if (onChange) {
  ...
  const handleAiDataApply = (aiData: ExecutiveSummarySBARData) => {
    if (onChange) {
      // Merge AI data with existing data
      onChange({
        situation: aiData.situation || data.situation,
  ...
  if (!data.situation && !data.background && !data.assessment && !data.recommendation && !data.asks?.length) {
    if (isEditMode) {
      return (
        <div className="p-6 border-2 border-dashed border-gray-300 rounded-xl text-center text-gray-500">
          ...
          <p className="text-sm font-roobert-medium">Executive Summary (SBAR) not configured</p>
          <p className="text-xs mt-1">Add Situation, Background, Assessment, and Recommendation in the properties panel</p>
        </div>
      );
    }
    return null;
  ...
  return (
    <div className="space-y-3">
      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-base font-roobert-bold text-white">Executive Summary (SBAR)</h3>
          </div>
      ...
      <div className="space-y-3">
        {/* 1. Situation */}
        {(isEditMode || data.situation) && (
          <div className="bg-white rounded-lg p-3 shadow-sm border-l-4" style={{ borderLeftColor: 'var(--accent-orange)' }}>
            <div className="flex gap-3">
              ...
              <div className="flex-1">
                ...
                {isEditMode ? (
                  <textarea
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg font-roobert-regular text-sm resize-none focus:outline-none focus:ring-2 focus:ring-orange-500"
                    rows={2}
                    placeholder="What's happening now? Current state or issue requiring attention..."
                  ...
                  <div className="text-sm text-gray-700 leading-relaxed font-roobert-regular whitespace-pre-wrap">
                    {renderWithExpressions(data.situation || '')}
                  </div>
                )}
              </div>
            </div>
              ...
              <div className="flex-1">
                ...
                {isEditMode ? (
                  <textarea
                    ...
                    rows={2}
                    placeholder="History and context behind the situation..."
      ...
      <AiPromptModal
        ...
        promptType="sbar"
        userData={data}
        componentName="Executive Summary (SBAR)"
...
export default ExecutiveSummarySBAR;
