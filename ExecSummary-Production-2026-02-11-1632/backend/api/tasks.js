  ...
  try {
    ...
    if (businessUnit) {
      ...
      filtered = filtered.filter(t => {
        const units = Array.isArray(t.businessUnits)
          ? t.businessUnits
          : t.businessUnit
            ? [t.businessUnit]
            : [];
        return units.some(unit => buValues.includes(unit));
      });
    }
...
const createTask = async (req, res) => {
  try {
    ...
    const normalizedBusinessUnits = Array.isArray(businessUnits)
      ? businessUnits
      : businessUnit
        ? [businessUnit]
        : [];
    ...
    const newTask = {
      ...
      businessUnit: businessUnit || normalizedBusinessUnits[0] || '',
      product: product || '',
...
const bulkDeleteTasks = async (req, res) => {
  try {
    ...
    // Filter out tasks to delete and track what was deleted
    ...
    taskIds.forEach(id => {
      if (!deletedTasks.find(t => t.id === id)) {
        notFoundIds.push(id);
      }
    });
    ...
    res.json({
      ...
      deletedTasks,
      notFoundIds,
      message: `Successfully deleted ${deletedTasks.length} task(s)`
    });
  } catch (error) {
