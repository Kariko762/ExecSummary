...
import VendorSummaryTemplate from '../templates/VendorSummaryTemplate';

interface VendorEditorProps {
  onClose: () => void;
}

export default function VendorEditor({ onClose }: VendorEditorProps) {
  ...
  const handleExportImage = async () => {
    ...
    try {
      ...
      link.download = `vendor-scorecard-${timestamp}.png`;
      link.href = dataUrl;
      link.click();
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/10 bg-gray-800/50">
          <div className="flex items-center gap-3">
            ...
            <span className="px-2 py-1 bg-cyan-500/20 text-cyan-400 text-xs font-roobert-medium rounded">
              PREVIEW
            </span>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <motion.button
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-lg bg-cyan-500/20 hover:bg-cyan-500/30 text-cyan-400 disabled:opacity-50"
        ...
        <div className="px-6 py-3 border-t border-white/10 bg-gray-800/50">
          <div className="flex items-center gap-2 text-xs text-white/60">
            ...
            <span>
              This is a preview of the vendor summary template. Inline editing and save functionality coming soon.
            </span>
          </div>
        </div>
      </motion.div>
            ...
            <div className="flex items-start gap-3 mb-4">
              ...
              <div>
                ...
                <p className="text-sm text-white/70 font-roobert-light">
                  The vendor summary template is currently in preview mode. Inline editing and save functionality will be added in the next update.
                </p>
              </div>
            </div>
            <button
              onClick={() => setShowSaveWarning(false)}
              className="w-full px-4 py-2 bg-cyan-500 hover:bg-cyan-600 text-white rounded-lg font-roobert-medium transition-colors"
            >
              Got it
            </button>
          </motion.div>
