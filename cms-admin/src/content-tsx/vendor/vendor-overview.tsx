...
interface VendorOverviewProps {
  data: any; // Accepts any vendor JSON data
  editMode?: boolean;
  onSave?: (data: any) => void;
}

// Export metadata - will be set dynamically
export const CONTENT_META = { id: 'vendor-overview', title: 'Vendor Technology Overview' };

// Main Component
export default function VendorOverview({ data, editMode = false, onSave }: VendorOverviewProps) {
  ...
  const handleExportImage = async () => {
    ...
    try {
      ...
      const vendorName = data?.metadata?.vendorName?.toLowerCase() || 'vendor';
      link.download = `${vendorName}-vendor-overview-${new Date().toISOString().split('T')[0]}.png`;
      link.href = dataUrl;
      link.click();
    } catch (error) {
...
function VendorOverviewHeader({ data, onExport, isExporting }: { 
  ...
  const logoFileName = vendorName === 'Synthesia' ? 'Synthesia_Logo_2024.png' : `${vendorName}.png`;
  const logoPath = `/vendor-logos/${logoFileName}`;
  const logoHeight = vendorName === 'Synthesia' ? 'h-8' : 'h-12';
...
function StorytellingIndex({ data }: { data: any }) {
...
function FeatureCards({ data }: { data: any }) {
...
function ProblemsSolved({ data }: { data: any }) {
  return (
    <div className="bg-white/5 backdrop-blur-sm rounded-lg p-6 border border-white/10">
      <div className="flex items-center gap-2 mb-5">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-white/60 font-roobert-light">Key pain points this vendor addresses</p>
        </div>
      </div>
...
function GTMHighlights({ data }: { data: any }) {
  return (
    <div className="bg-white/5 backdrop-blur-sm rounded-lg p-6 border border-white/10">
      <div className="flex items-center gap-2 mb-5">
