...
interface ExecutiveSummaryCPSARData {
  context?: string;
  problem?: string;
  solution?: string;
...
interface ExecutiveSummaryCPSARProps {
  data: ExecutiveSummaryCPSARData;
  isEditMode?: boolean;
  onChange?: (newData: ExecutiveSummaryCPSARData) => void;
}

export const ExecutiveSummaryCPSAR: React.FC<ExecutiveSummaryCPSARProps> = ({ 
  data,
  isEditMode = false,
  onChange
}) => {
  ...
  const handleAiDataApply = (aiData: ExecutiveSummaryCPSARData) => {
    if (onChange) {
      // Merge AI data with existing data
      onChange({
        context: aiData.context || data.context,
        problem: aiData.problem || data.problem,
        solution: aiData.solution || data.solution,
  ...
  return (
    <div className="space-y-3">
      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-base font-roobert-bold text-white">Executive Summary (CPSAR)</h3>
          </div>
              ...
              <div className="flex-1">
                ...
                {isEditMode ? (
                  <textarea
                    ...
                    rows={2}
                    placeholder="What's the backdrop? Quarter progress, strategic priorities, external factors..."
                    value={data.context || ''}
                    onChange={(e) => updateField('context', e.target.value)}
                  />
                ) : (
                  <div className="text-sm text-gray-700 leading-relaxed font-roobert-regular whitespace-pre-wrap">
                    {renderWithExpressions(data.context || '')}
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="bg-white rounded-lg p-3 shadow-sm border-l-4" style={{ borderLeftColor: 'var(--accent-red, #ef4444)' }}>
            <div className="flex gap-3">
              ...
              <div className="flex-1">
                ...
                {isEditMode ? (
                  <textarea
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg font-roobert-regular text-sm resize-none focus:outline-none focus:ring-2"
                    style={{ '--tw-ring-color': 'var(--accent-red, #ef4444)' } as React.CSSProperties}
                    rows={2}
                    placeholder="What's broken? Risk factors, blockers, critical issues requiring action..."
                    value={data.problem || ''}
                    onChange={(e) => updateField('problem', e.target.value)}
                  />
                ) : (
                  <div className="text-sm text-gray-700 leading-relaxed font-roobert-regular whitespace-pre-wrap">
                    {renderWithExpressions(data.problem || '')}
                  </div>
                )}
              </div>
            </div>
              ...
              <div className="flex-1">
                ...
                {isEditMode ? (
                  <textarea
                    ...
                    rows={2}
                    placeholder="What was done? Immediate actions taken, resources deployed, tactical fixes..."
                    value={data.solution || ''}
                    onChange={(e) => updateField('solution', e.target.value)}
                  />
                ) : (
                  <div className="text-sm text-gray-700 leading-relaxed font-roobert-regular whitespace-pre-wrap">
                    {renderWithExpressions(data.solution || '')}
                  </div>
                )}
              </div>
            </div>
      ...
      <AiPromptModal
        ...
        promptType="cpsar"
        userData={data}
        componentName="Executive Summary (CPSAR)"
...
export default ExecutiveSummaryCPSAR;
