...
interface ExecutiveSummaryPyramidData {
  mainArgument?: string;
  keyPoints?: string[];
...
interface ExecutiveSummaryPyramidProps {
  data: ExecutiveSummaryPyramidData;
  isEditMode?: boolean;
  onChange?: (newData: ExecutiveSummaryPyramidData) => void;
}

export const ExecutiveSummaryPyramid: React.FC<ExecutiveSummaryPyramidProps> = ({ 
  data,
  isEditMode = false,
  onChange
}) => {
  ...
  const updateField = (field: keyof ExecutiveSummaryPyramidData, value: any) => {
    if (onChange) {
  ...
  const handleAiDataApply = (aiData: ExecutiveSummaryPyramidData) => {
    if (onChange) {
      // Merge AI data with existing data
      onChange({
        mainArgument: aiData.mainArgument || data.mainArgument,
        keyPoints: aiData.keyPoints || data.keyPoints,
        supportingDetails: aiData.supportingDetails || data.supportingDetails,
        nextSteps: aiData.nextSteps || data.nextSteps,
  ...
  if (!data.mainArgument && !data.keyPoints?.length && !data.supportingDetails && !data.nextSteps && !data.asks?.length) {
    if (isEditMode) {
      return (
        <div className="p-6 border-2 border-dashed border-gray-300 rounded-xl text-center text-gray-500">
          ...
          <p className="text-sm font-roobert-medium">Executive Summary (Pyramid) not configured</p>
          <p className="text-xs mt-1">Add Main Argument, Key Points, Supporting Details, and Next Steps in the properties panel</p>
        </div>
      );
    }
    return null;
  ...
  return (
    <div className="space-y-3">
      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-base font-roobert-bold text-white">Executive Summary (Pyramid)</h3>
          </div>
      ...
      <div className="space-y-3">
              ...
              <div className="flex-1">
                ...
                {isEditMode ? (
                  <textarea
                    ...
                    rows={2}
                    placeholder="The primary conclusion or recommendation at the top of the pyramid..."
                    value={data.mainArgument || ''}
                    onChange={(e) => updateField('mainArgument', e.target.value)}
                  />
                ) : (
                  <div className="text-sm text-gray-700 leading-relaxed font-roobert-semibold whitespace-pre-wrap">
                    {renderWithExpressions(data.mainArgument || '')}
                  </div>
                )}
              </div>
            </div>
        ...
        {/* 2. Key Points */}
        {(isEditMode || (data.keyPoints && data.keyPoints.length > 0)) && (
              ...
              <div className="flex-1">
                ...
                {isEditMode ? (
                  <div className="space-y-2">
                    {(data.keyPoints || []).map((point, index) => (
                      <div key={index} className="flex gap-2">
                        <input
                          type="text"
                          className="flex-1 px-3 py-2 border border-gray-300 rounded-lg font-roobert-regular text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                  ...
                  <ul className="space-y-1">
                    {data.keyPoints?.map((point, index) => (
                      <li key={index} className="text-sm text-gray-700 leading-relaxed font-roobert-regular flex gap-2">
                        <span style={{ color: 'var(--accent-blue)' }}>•</span>
                        <span>{renderWithExpressions(point || '')}</span>
                      </li>
                    ))}
                  </ul>
                )}
        ...
        {(isEditMode || data.supportingDetails) && (
              ...
              <div className="flex-1">
                ...
                {isEditMode ? (
                  <textarea
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg font-roobert-regular text-sm resize-none focus:outline-none focus:ring-2 focus:ring-purple-500"
                    rows={2}
                    placeholder="Evidence, data, and details that support the key points..."
                    value={data.supportingDetails || ''}
                    onChange={(e) => updateField('supportingDetails', e.target.value)}
                  />
                ) : (
                  <div className="text-sm text-gray-700 leading-relaxed font-roobert-regular whitespace-pre-wrap">
                    {renderWithExpressions(data.supportingDetails || '')}
                  </div>
                )}
              </div>
            </div>
        ...
        {/* 4. Next Steps */}
        {(isEditMode || data.nextSteps) && (
          <div className="bg-white rounded-lg p-3 shadow-sm border-l-4" style={{ borderLeftColor: 'var(--accent-green)' }}>
            <div className="flex gap-3">
              <div className="flex-shrink-0">
                <div 
                  className="w-7 h-7 rounded-lg flex items-center justify-center font-roobert-bold text-white"
              ...
              <div className="flex-1">
                ...
                {isEditMode ? (
                  <textarea
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg font-roobert-regular text-sm resize-none focus:outline-none focus:ring-2 focus:ring-green-500"
                    rows={2}
                    placeholder="Action items and implementation plan..."
                    value={data.nextSteps || ''}
                    onChange={(e) => updateField('nextSteps', e.target.value)}
                  />
                ) : (
                  <div className="text-sm text-gray-700 leading-relaxed font-roobert-regular whitespace-pre-wrap">
                    {renderWithExpressions(data.nextSteps || '')}
                  </div>
                )}
              </div>
            </div>
      ...
      <AiPromptModal
        ...
        promptType="pyramid"
        userData={data}
        componentName="Executive Summary (Pyramid)"
...
export default ExecutiveSummaryPyramid;
