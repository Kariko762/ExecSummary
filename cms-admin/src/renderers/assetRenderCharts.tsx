...
export const RadialProgressPattern: React.FC<ChartPatternProps> = ({ data, onChange, mode }) => {
  ...
  // Display mode
  const COLORS = ['var(--brand-primary)', 'var(--brand-secondary)', 'var(--accent-blue)', 'var(--accent-green)', 'var(--accent-yellow)'];
  ...
  if (!isArray && data) {
    ...
    const getColor = (status: string) => {
      switch(status) {
        case 'onTrack': return 'var(--semantic-success)';
        case 'atRisk': return 'var(--semantic-warning)';
        case 'blocked': return 'var(--semantic-error)';
