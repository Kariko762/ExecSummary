...
export const ContentModalFixedMenu: React.FC<ContentModalFixedMenuProps> = ({ content, onClose }) => {
  ...
  const exportAsImage = async () => {
    ...
    try {
      // Hide all header buttons during export
      ...
      // Get the export wrapper (includes header + content)
  ...
  const scrollToSection = (sectionKey: string) => {
    ...
    if (element) {
      // Account for header height and add extra spacing for better positioning
      const yOffset = -124; // Extra spacing (adjusted by ~100px for better anchor positioning)
...
{JSON.stringify({
  ...
  status: content.status
            ...
            <div ref={exportWrapperRef} className="flex flex-col flex-1 overflow-hidden">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="relative export-button">
                      <button
                        onClick={() => setShowExportMenu(!showExportMenu)}
                        disabled={isExporting}
                        className="px-4 py-2 rounded-xl bg-white/10 hover:bg-white/20 text-white text-sm font-roobert-medium flex items-center gap-2 transition-all disabled:opacity-50"
                      >
                        {isExporting ? (
                          <Loader2 className="w-4 h-4 animate-spin" />
                        ) : (
                    ...
                    <button
                      onClick={onClose}
                      className="close-button w-10 h-10 rounded-xl bg-white/10 hover:bg-white/20 text-white transition-colors flex items-center justify-center"
                    >
                      <X className="w-5 h-5" />
                    </button>
                  </div>
              ...
              {!isDraft && sections.filter(s => s.type !== 'hr').length > 1 && (
                <div className="w-52 bg-gradient-to-b from-fis-eggplant to-fis-raspberry overflow-y-auto flex-shrink-0">
                  <nav className={isFullscreen ? 'p-2 space-y-0' : 'p-3 space-y-0.5'}>
                    {sections.filter(section => section.type !== 'hr').map((section) => (
                      <button
                        key={section.key}
                        onClick={() => scrollToSection(section.key)}
                        className={`
                          w-full text-left rounded-lg font-roobert-medium text-xs
                          transition-all duration-200 flex items-center gap-2
                          ${isFullscreen ? 'px-2 py-1' : 'px-2 py-1.5'}
