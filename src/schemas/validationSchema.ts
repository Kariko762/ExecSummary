 ...
 * Defines validation rules for each renderer type
 */
...
export const validationSchemas: Record<string, TypeValidation> = {
      ...
      },
      {
        ...
        message: (data) => {
          ...
          return allObjects
            ...
            : '✗ Contains non-object items';
        },
        severity: 'success'
      },
      {
        field: 'Fields Schema',
        check: (data, content, sectionKey) => !!content[`_${sectionKey}_fields`],
        message: (data, content, sectionKey) => {
  ...
  // Metric Cards (array of objects with label/value)
  metricCards: {
    type: 'metricCards',
    rules: [
      {
      ...
      },
      {
        ...
        message: (data) => {
          ...
          return validStructure
            ? '✓ All metrics have label/value'
            : '⚠ Some metrics missing label/value';
        },
        severity: 'success'
      },
      {
    ...
    rules: [
      ...
      },
      {
        ...
        message: (data) => {
          ...
          return validStructure
            ? '✓ All items have name & value'
            : '⚠ Missing name/value in some items';
        },
        severity: 'success'
      },
      {
        field: 'Chart Config',
        check: (data, content, sectionKey) => !!content[`_${sectionKey}_chartConfig`],
        message: (data, content, sectionKey) => {
          const config = content[`_${sectionKey}_chartConfig`];
          return config
            ? `✓ Chart config defined`
            : '⚠ Missing _chartConfig metadata';
        },
        severity: 'success'
      }
    ]
...
/**
 * Validate a section using its type schema
 */
